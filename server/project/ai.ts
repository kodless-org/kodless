...
const CONCEPT_GENIE_PROMPT = fs.readFileSync(
  "prompts/concept_genie.txt",
  "utf-8"
);
const CONCEPT_SPECTOR_PROMPT = fs.readFileSync(
  "prompts/concept_spector.txt",
  "utf-8"
);
...
const FRONTEND_GENIE_PROMPT = fs.readFileSync(
  "prompts/frontend_genie.txt",
  "utf-8"
);
...
const assistants: Record<Assistant, string> = {
  ...
  frontend_genie: FRONTEND_GENIE_PROMPT,
};
...
export const generateUpdatedConcept = async (
  concept: string,
  conceptSrc: string,
  prompt: string
) => {
...
export const generateAppDefinition = async (
  spec: string,
  currentApp: string,
  prompt: string
) => {
...
export const generateUpdatedRoute = async (
  spec: string,
  appDefinition: string,
  routeSrc: string,
  prompt: string
